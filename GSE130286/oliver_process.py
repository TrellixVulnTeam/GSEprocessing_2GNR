...
class NCBIGeo:
    ...
    def make_expr(self, gpl, takeLog=False):
    ...
    def export(self, df, gpl, csv=False):
        if csv:
            survival_filename = f"{self.accessionID}-{gpl.name}-survival.csv"
            df.to_csv(survival_filename)
        else:
    ...
    def export_all(self):
        ...
        for _, gpl in self.gse.gpls.items():
            expr = self.make_expr(gpl, takeLog=True)
            idx = self.make_idx(gpl)
            survival = self.make_survival(gpl)
            ih = self.make_ih(gpl)
